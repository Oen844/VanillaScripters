...
.then(data => {
  ...
  data.forEach(room => {
     let roomElement = `
      <div data-drop-target="true" class="room-card" id="room_${room._id}" style="background-image: url('../img/${room.artwork.thumbnail}')">
        <h2 id="titulo">${room.name}</h2>
        <button onclick="joinRoom(${room._id})" class="bubbly-button join-btn"  id="boton${room._id}" data-id="${room._id}" disabled>Unete!</button>
      </div>`;
